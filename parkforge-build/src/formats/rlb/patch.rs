    ...
    pub(crate) fn check(&self, path: &Path, config: &BuildConfig) -> Result<()> {
        ...
        for edit in &self.edits {
            for (field, value) in &edit.set {
                ...
                drop(
                    value.resolve(config).map_err(|error| {
                        error.into_build_error(path, &edit.table, edit.row, field)
                    })?,
                );
            }
        }
        ...
        for edit in &self.edits {
            ...
            for (field, source_value) in &edit.set {
                let value = source_value
                    .resolve(config)
                    .map_err(|error| error.into_build_error(path, &edit.table, edit.row, field))?;
